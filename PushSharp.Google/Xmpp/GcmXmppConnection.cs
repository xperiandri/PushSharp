...
namespace PushSharp.Google
{
    public class GcmXmppConnection
    {
        ...
        private readonly XNamespace GCM_NS = GCM_MSG_NAMESPACE;
        ...
        public async Task<bool> ConnectAsync()
        {
            ...
            await sslStream.AuthenticateAsClientAsync(Configuration.Host, certificates, System.Security.Authentication.SslProtocols.Tls, false);
            stream = sslStream;
            ...
            Task.Factory.StartNew(() => Listen(authCompletion));
        ...
        public void Authenticate()
        {
            ...
            WriteElementAsync(el);
        }

        public async Task SendAsync(CompletableNotification notification)
        {
        ...
        private void Listen(TaskCompletionSource<bool> authCompletion)
        {
            try
            {
        ...
        private Task WriteElementAsync(XElement element)
        {
            Log.Debug("GCM-XMPP: Sending: " + element);

            element.WriteTo(xml);
            return xml.FlushAsync();
        }
